...
public class SampleDetection extends OpenCvPipeline {
    ...
    public SampleDetection(String c){
        this.color = c;
    }
    ...
    public Mat processFrame(Mat input) {
        ...
        // Process red, blue, and yellow - ORIGINAL CODE WITHOUT BLUE-RED DISTINCTION
        ...
        if(this.color.equals("blue")){ //draws boxes around blue and yellow
            detectAndDrawRectangles(input, maskBlue, new Scalar(0, 0, 255)); // Blue bounding box
            detectAndDrawRectangles(input, maskYellow, new Scalar(0, 255, 255)); // Yellow bounding box
        } else if(this.color.equals("red")){ //draws boxes around red and yellow
            detectAndDrawRectangles(input, maskRed, new Scalar(255, 0, 0)); // Red bounding box
            detectAndDrawRectangles(input, maskYellow, new Scalar(0, 255, 255)); // Yellow bounding box
        } else if(this.color == null){ //draws boxes around blue, red, and yellow
            detectAndDrawRectangles(input, maskRed, new Scalar(255, 0, 0)); // Red bounding box
            detectAndDrawRectangles(input, maskBlue, new Scalar(0, 0, 255)); // Blue bounding box
            detectAndDrawRectangles(input, maskYellow, new Scalar(0, 255, 255)); // Yellow bounding box
        }
